...
;(function ($) {
	...
	String.prototype.parse = function parse_string (re, vars) {
		vars.__time = (new Date()).getTime();
		return this.replace(re, (o, m) => vars[m]);
	};
	...
	String.prototype.extract = function extract_parts (re) {
		return this.split(re).filter(s => s);
	};
	...
	var get_uid = function get_uid (el) {
		...
		//console.log(id, el[0]);
		return {id: id, ns: (data.source.replace(/[\/|\.]/g, '_') + id + 'prv').reverse()};
	};
	...
	var init_receivers = function init_receivers () {
		$('[data-provider]').each(function () {
			var $this = $(this), d = $this.data(), src = d.provider.split('@', 2), source = src[0], node = src[1];
	...
	var init_request = function init_request (provider) {
		var ep = provider.data.source.extract(/.*(get|post)\.(xml|json).*/g), method = ep[0], format = ep[1];
	...
	var DataReceiver = function (el, provider) {
		...
		receiver = {
			...
			dataReceived: function (response) {
				var result = value_of(response, data.node);
			...
			parseRow: function (row) {
				...
				snippet.html(el.html().parse(/@(\w+)/gim, row));
			}
		};
